...
- MVP Scopes: `openid`, `email`, `profile`
- 후순위 Scopes (incremental authorization): `calendar`, `tasks` — 해당 기능 구현 시 추가 요청
- `google_refresh_token` 저장 → 캘린더/태스크 연동 (후순위)
...
X-API-Key는 MVP에서 단일 admin 키 (`OPENCAMPUS_API_KEY`). MCP/OpenClaw에서 사용 시 `X-User-Id` 헤더로 대상 유저를 지정. 추후 per-user API key 테이블로 전환.
...
CREATE TABLE users (
  ...
  canvas_access_token TEXT,           -- per-user Canvas PAT (encrypted)
...
Canvas 과목 동기화 시 자동 생성 (enrollment_term → semester 매핑).
...
> **참고**: LearningX/LCMS 영상 다운로드는 LTI 1.0 OAuth → xn_api_token JWT → LCMS session → VOD MP4의 6단계 인증 체인이 필요. 레거시 `video_dl.py`에 구현되어 있으며 TypeScript 포팅 시 cookie-based session + XML parsing이 복잡. MVP에서는 출석 조회(`getAttendanceItems`)를 우선 구현하고, 영상 다운로드는 Phase 2로 분리.
...
PATCH /users/me              프로필 수정 (canvas_access_token 등)
```
...
```
GET  /courses/:id/weeks      주차 목록 (자료 없는 주차도 포함)
```
...
```
GET  /jobs                           내 작업 목록 (?status=&material_id=)
GET  /jobs/:id/stream                SSE 진행률 (canonical SSE endpoint)
```
...
MVP: 주기적 폴링 (매 분 `setInterval`로 upcoming deadlines 조회 → 알림 생성). pod 재시작 시 스케줄 소실되나, 다음 폴링에서 복구됨. 추후 pg-boss로 전환하여 reliable scheduling.
...
- **TLS**: `api.opencampus.json-server.win`은 `proxied=true` (Cloudflare). Cloudflare 무료 Universal SSL은 1단계 서브도메인만 커버하므로, `api.opencampus`는 2단계 → Cloudflare Advanced Certificate Manager 또는 `proxied=false` + cert-manager DNS01로 전환 필요. 배포 후 TLS 확인 필수.
- **LibreOffice**: PPT 변환에 `libreoffice-headless` 필요. API Docker 이미지에 포함하면 ~400MB 증가. MVP에서는 PPT 처리를 후순위로 미루거나, 별도 worker 이미지로 분리.
- **Prisma**: `@updatedAt` annotation으로 `updated_at` 자동 관리
- **Pagination**: 모든 목록 엔드포인트에 `?page=1&limit=20` 표준 적용 (cursor-based는 추후)

### course_weeks 자동 생성

`course_weeks`는 material 업로드 시 해당 주차가 없으면 자동 upsert. 별도 생성 엔드포인트 불필요.
